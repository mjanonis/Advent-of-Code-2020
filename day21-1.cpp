...
int main()
{
  ...
  while (!undiscovered_allergens.empty()) {
    ...
    for (auto& ati : allergen_to_ingredients) {
      if (ati.second.size() == 1) {
        ...
        allergen_name[*ati.second.begin()] = ati.first;
  ...
  size_t non_allergens =
      std::count_if(ingredients.begin(), ingredients.end(),
                    [allergen_name](const std::string& i) {
                      return allergen_name.find(i) == allergen_name.end();
                    });
  output << non_allergens << std::endl;
}
