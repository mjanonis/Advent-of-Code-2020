...
// Yes, this could alternatively be done using regex
bool is_valid(const std::string& key, const std::string& value)
{
  ...
  if (key == "hgt") {
    ...
    if (unit == "cm" && std::stoi(height) >= 150 && std::stoi(height) <= 193) {
      return true;
    }
    if (unit == "in" && std::stoi(height) >= 59 && std::stoi(height) <= 76) {
      return true;
    }
    return false;
  }
  if (key == "hcl") {
    if (value.length() == 7 && value[0] == '#' && isxdigit(value[1]) &&
        ...
        isxdigit(value[4]) && isxdigit(value[5]) && isxdigit(value[6])) {
      return true;
    }
    return false;
  }
  ...
  if (key == "pid") {
    if (value.length() == 9 && isdigit(value[0]) && isdigit(value[1]) &&
        isdigit(value[2]) && isdigit(value[3]) && isdigit(value[4]) &&
        isdigit(value[5]) && isdigit(value[6]) && isdigit(value[7]) &&
        isdigit(value[8])) {
      return true;
    }
    return false;
